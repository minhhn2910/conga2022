...
parser.add_argument('--gpu', default=None, type=int,
                    help='GPU id to use.')
parser.add_argument('--scale-weight', default=1.0, type=float,
                    help='Scale weight before converting to posit')
parser.add_argument('--scale-act', default=1.0, type=float,
                    help='Scale act before converting to posit')
parser.add_argument('--nsize', default=8, type=int,
                    help='nsize')
parser.add_argument('--es', default=1, type=int,
                    help='es')

parser.add_argument('--int-log', default=3, type=int,
                    help='nsize')
parser.add_argument('--frac-log', default=1, type=int,
                    help='es')
...
#first and last layer of the network. Todo: automate this
exclude_list = [0,53]
weight_table = []
act_table = []
...
def main():
    ...
    scale_act = args.scale_act
    nsize= args.nsize
    es = args.es
    int_log = args.int_log
    frac_log = args.frac_log
    print ("running posit inference with scale weight %f scale act %f"%(scale_weight,scale_act))
    print (" int frac ", int_log, frac_log)
    weight_table = generate_log2system(int_log,frac_log)
    act_table = generate_log2system(int_log,frac_log)
...
def linear_weight(input):
    input = configurable_table_quantize(input,torch.tensor(weight_table, dtype=torch.float), scale= 1.0)
    return input
def linear_activation(input):
    global act_data
    input = configurable_table_quantize(input,torch.tensor(act_table, dtype=torch.float), scale= 1.0)
    return input
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        layer_count = 0
        total_layer = 0
        print ("posit conf ", nsize, es, scale_weight,scale_act)
        print ("exclude layers ", exclude_list)
        for name, module in model.named_modules():
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear) :
                #resnet
                if layer_count not in exclude_list:#(layer_count !=0 and layer_count < 53):
