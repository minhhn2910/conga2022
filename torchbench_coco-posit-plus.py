...
parser = argparse.ArgumentParser(description='PyTorch COCO bench')
...
print ('scale_w %f scale_a %f',scale_weight, scale_act )
...
def linear_weight(input):
    input = posit_quantize(input, nsize=6, es=1, scale= scale_weight)
    return input
def linear_activation(input):
    global act_data
    input = posit_quantize(input, nsize=6, es=1, scale= scale_act)
    return input
