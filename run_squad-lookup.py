...
def main():
    ...
    parser.add_argument('--nsize', default=6, type=int,
                    help='nsize ')
    parser.add_argument('--es', default=2, type=int,
                    help='es')
    parser.add_argument('--int_log', default=3, type=int,
                    help='int log ')
    parser.add_argument('--frac_log', default=1, type=int,
                    help='frac_log ')
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            inT_ = args.int_log
            fraC_ = args.frac_log
            print ('int frac : ', inT_ , fraC_)
            weight_table = generate_log2system(inT_,fraC_)
            act_table  = generate_log2system(inT_,fraC_)
            ...
            def linear_weight(input):
                ...
                #input = posit_quantize(input, nsize=nsize, es=es, scale= scale_weight)
                return input
            def linear_activation(input):
                global act_data
                input = configurable_table_quantize(input,torch.tensor(act_table, dtype=torch.float), scale= 1.0)
                #input = posit_quantize(input, nsize=nsize, es=es, scale= scale_act)
                return input
            ...
            for name, module in model.named_modules():
                if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear) :
                    if (layer_count !=0 and layer_count < 67):
