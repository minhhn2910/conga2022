...
parser.add_argument('--skip', default='', type=str,
                    help='list of layer to skip, separated by comma .')
...
def main():
    ...
    posit_nsize = args.nsize
    posit_es = args.es
    if len(args.skip) > 0:
        temp = args.skip.split(',')
        for item in temp:
            skip_layers.append(int(item))
    ...
    if args.seed is not None:
        ...
        warnings.warn('You have chosen to seed training. '
                      ...
                      'from checkpoints.')
    print ("posit parameters : nsize %d  es %d  scale_weight %f   scale_act %f " %(posit_nsize, posit_es, scale_weight, scale_act))
    print (" list of layers to be skipped : ", skip_layers)
...
def linear_weight(input):
    input = posit_quantize(input, nsize=posit_nsize, es=posit_es, scale= scale_weight)
    return input
def linear_activation(input):
    global act_data
    input = posit_quantize(input, nsize=posit_nsize, es=posit_es, scale= scale_act)
    return input
    ...
    if args.pretrained:
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear) :
                if (layer_count not in skip_layers):
